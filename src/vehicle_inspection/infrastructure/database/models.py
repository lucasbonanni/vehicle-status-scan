...
class InspectorModel(Base):
    ...
    role = Column(String(50), nullable=False, default=InspectorRole.JUNIOR.value)
    license_number = Column(String(50), nullable=False, unique=True, index=True)
    status = Column(String(50), nullable=False, default=InspectorStatus.ACTIVE.value)
    is_active = Column(Boolean, nullable=False, default=True)
...
class InspectionModel(Base):
    ...
    )  # Not unique - allows multiple inspections per vehicle
    ...
    checkpoint_scores = Column(
        JSON, nullable=True
    )  # JSON array of checkpoint score objects

    # Calculated safety metrics
    total_score = Column(
        Numeric(precision=5, scale=2), nullable=True
    )  # Total score calculated from checkpoints
    is_safe = Column(
        Boolean, nullable=True
    )  # Whether vehicle is safe (≥80 total score)
    requires_reinspection = Column(
        Boolean, nullable=True
    )  # Whether vehicle requires reinspection

    # Inspector observations
    observations = Column(Text, nullable=False, default="")

    # Inspection status
    status = Column(String(50), nullable=False, default=InspectionStatus.DRAFT.value)

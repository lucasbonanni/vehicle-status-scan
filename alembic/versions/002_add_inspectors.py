...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_inspectors_license_number"),
        "inspectors",
        ["license_number"],
        unique=True,
    )
    ...
    op.execute(
        "ALTER TABLE inspectors ADD CONSTRAINT check_role CHECK (role IN ('junior', 'senior', 'supervisor'))"
    )
    op.execute(
        "ALTER TABLE inspectors ADD CONSTRAINT check_status CHECK (status IN ('active', 'inactive', 'suspended'))"
    )

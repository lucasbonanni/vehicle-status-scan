...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "checkpoint_scores", postgresql.JSON(astext_type=sa.Text()), nullable=True
        ),
        ...
        sa.ForeignKeyConstraint(
            ["inspector_id"], ["inspectors.id"], name="fk_inspections_inspector_id"
        ),
    )

    # Create indexes for performance
    op.create_index(
        op.f("ix_inspections_license_plate"),
        "inspections",
        ["license_plate"],
        unique=False,
    )
    op.create_index(
        op.f("ix_inspections_inspector_id"),
        "inspections",
        ["inspector_id"],
        unique=False,
    )
    ...
    op.execute(
        "ALTER TABLE inspections ADD CONSTRAINT check_vehicle_type CHECK (vehicle_type IN ('car', 'motorcycle'))"
    )
    op.execute(
        "ALTER TABLE inspections ADD CONSTRAINT check_inspection_status CHECK (status IN ('draft', 'completed'))"
    )
